...
public class Test extends JFrame {
    ...
    public Test() {
        ...
        button.addActionListener(e -> {
            ModalDialog.showModal(this, new AdaptSimpleModalBorder(new SimpleInputForms(), "Input", AdaptSimpleModalBorder.YES_NO_OPTION, (controller, action) -> {
                System.out.println(action);
                if (action == AdaptSimpleModalBorder.YES_OPTION) {
                    controller.consume();
                    ModalDialog.pushModal(new AdaptSimpleModalBorder(new SimpleInputForms2(), "New Input", AdaptSimpleModalBorder.YES_NO_OPTION, (controller1, action1) -> {
                    }), "input");
                }
            }), "input");
        });
