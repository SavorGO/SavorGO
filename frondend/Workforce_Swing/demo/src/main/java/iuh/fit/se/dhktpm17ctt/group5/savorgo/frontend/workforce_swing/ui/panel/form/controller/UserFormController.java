...
import iuh.fit.se.dhktpm17ctt.group5.savorgo.frontend.workforce_swing.ui.scrollpane.popupform.create.UserCreateInputForm;
import iuh.fit.se.dhktpm17ctt.group5.savorgo.frontend.workforce_swing.ui.scrollpane.popupform.info.InfoFormUser ;
import iuh.fit.se.dhktpm17ctt.group5.savorgo.frontend.workforce_swing.ui.scrollpane.popupform.update.UserUpdateInputForm;
...
public class UserFormController {
    ...
    private void showDetailsModal() {
        ...
        InfoFormUser  infoFormUser  = createInfoFormUser (userId);
        ModalDialog.showModal(formUser, new AdaptSimpleModalBorder(infoFormUser, "User details information", AdaptSimpleModalBorder.DEFAULT_OPTION, (controller, action) -> {}), DefaultComponent.getInfoForm());
    }
    ...
    private void showCreateModal() {
        UserCreateInputForm  userCreateInputForm  = new UserCreateInputForm();
        ModalDialog.showModal(formUser, new AdaptSimpleModalBorder(userCreateInputForm, "Create user", AdaptSimpleModalBorder.YES_NO_OPTION, (controller, action) -> {
            if (action == AdaptSimpleModalBorder.YES_OPTION) {
                handleCreateUser (userCreateInputForm );
            }
     ...
     */
    private void handleCreateUser (UserCreateInputForm  inputFormCreateUser ) {
        Object[] userData = inputFormCreateUser .getData();
        try {
    ...
    private void showEditModal() {
        ...
        UserUpdateInputForm  userUpdateInputForm  = new UserUpdateInputForm();
        ModalDialog.showModal(formUser, new AdaptSimpleModalBorder(userUpdateInputForm, "Update user", AdaptSimpleModalBorder.YES_NO_OPTION, (controller, action) -> {
            if (action == AdaptSimpleModalBorder.YES_OPTION) {
               handleUpdateUser (userUpdateInputForm );
            }
        }), DefaultComponent.getInputForm());
    }
     ...
     */
    private void handleUpdateUser (UserUpdateInputForm  userUpdateInputForm ) {
        Object[] userData;
		try {
			userData = userUpdateInputForm .getData();
     ...
     */
    private InfoFormUser createInfoFormUser (String userId) {
        try {
            return new InfoFormUser (userId);
        } catch (IOException e) {
